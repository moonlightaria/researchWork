...
void Slic::init_data(IplImage *image) {
    /* Initialize the cluster and distance matstump2s. */
...
std::vector<std::vector<int>> Slic::PixelSLICid() {
	return clusters;
}
int Slic::centersSize() {
	return centers.size();
}
std::vector<std::vector<double>> Slic::getCenters() {
	return centers;
}